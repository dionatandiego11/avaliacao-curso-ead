...
const featuredUniversities: University[] = [
    { name: 'Loughborough University', rating: 4.2, reviewCount: 175 },
    { name: 'Nottingham Trent University (NTU)', rating: 4.3, reviewCount: 93 },
    { name: 'University of Birmingham', rating: 4.4, reviewCount: 67 },
    { name: 'University of Liverpool', rating: 4.3, reviewCount: 54 },
    { name: 'Queen Mary University of London (QMUL)', rating: 4.1, reviewCount: 51 },
];

const UniversityReviewItem: React.FC<{ university: University }> = ({ university }) => {
    return (
        <div className="bg-white p-3 rounded-md flex items-center space-x-4 hover:bg-gray-100 transition-colors cursor-pointer">
            <div className="bg-yellow-400 text-white p-3 rounded-md flex items-center space-x-2">
                ...
                <span className="font-bold text-lg">{university.rating.toFixed(1)}</span>
            </div>
            <div>
                <h4 className="font-bold text-gray-800">{university.name}</h4>
                <p className="text-sm text-gray-500">{university.reviewCount} avaliações recentes</p>
            </div>
        </div>
    );
};
    ...
    <section className="py-16 sm:py-24">
      <div className="container mx-auto px-6 text-center">
        ...
        <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mt-2">
            Universidades Avaliadas Recentemente
        </h2>
        <p className="max-w-3xl mx-auto text-gray-600 mt-4">
            Avaliações são uma ferramenta útil para ajudar estudantes a tomar a decisão certa sobre onde estudar. É por isso que usamos apenas avaliações 100% verificadas de estudantes, baseadas nas experiências de estudantes reais de todo o Brasil.
        </p>
                ...
                <div className="md:w-1/2 w-full">
                    <div className="space-y-3 text-left">
                        {featuredUniversities.map((uni) => (
                           <UniversityReviewItem key={uni.name} university={uni} />
                        ))}
                    </div>
                </div>
            </div>
